...
const FacultyTable = ({
  ...
  const getSortedData = () => {
    ...
    return [...facultyData].sort((a, b) => {
      ...
      const percentageA = (summaryA.presentDays / totalDays) * 100;
      const percentageB = (summaryB.presentDays / totalDays) * 100;
  ...
  return (
    <div className="w-full flex flex-col border border-slate-200 rounded-lg overflow-hidden">
      ...
      <div
        ref={headerRef}
        className="flex-shrink-0 bg-slate-50 border-b border-slate-200 overflow-x-auto"
              ...
              <div key={index} className="flex border-b border-slate-200 hover:bg-slate-50 transition-colors duration-150 min-w-max">
                ...
                <div className="w-48 px-4 py-3 whitespace-nowrap text-center border-r border-slate-200">
                  <div className="flex flex-col items-center space-y-1">
                    <span className={`inline-flex items-center px-3 py-2 rounded-full text-sm font-medium ${getStatsBackgroundColor(percentage)}`}>
                      {summary.presentDays}/{totalDays}
                    </span>
                ...
                <div className="w-32 px-4 py-3 whitespace-nowrap text-center">
                  <div className="flex flex-col items-center space-y-1">
                    <span className={`text-lg font-bold ${percentage >= 75 ? 'text-emerald-700' :
                        percentage >= 50 ? 'text-amber-700' : 'text-rose-700'
                      }`}>
                      {percentage}%
                    </span>
                  </div>
