...
export const generateReportHTML = (reportData) => {
  ...
  const rowsHtml = dailyAttendance.map(d => {
    ...
    const inTime = d.inTime || (isWeekend ? 'Weekend' : isHoliday ? 'Holiday' : '-');
    const outTime = d.outTime || (isWeekend ? 'Weekend' : isHoliday ? 'Holiday' : '-');
    ...
    return `
      <tr>
        ...
        <td style="padding: 8px 12px; border-bottom: 1px solid #e2e8f0;">
          <span style="font-weight: 600; color: ${d.status === 'Present' ? '#059669' : d.status === 'Absent' ? '#dc2626' : '#64748b'}">
            ${d.status}
          </span>
        </td>
...
<body>
  <div class="page">
    ...
    <div class="footer">
      Generated by e-Office APFRS | Page 1 of 2
    </div>
  </div>
  ...
  <div class="page">
    ...
    <div class="footer">
      Generated by e-Office APFRS | Page 2 of 2
    </div>
  </div>
</body>
</html>
